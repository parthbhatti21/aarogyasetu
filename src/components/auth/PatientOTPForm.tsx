...
import { HospitalSelector } from '@/components/patient/HospitalSelector';
...
const PatientOTPForm = ({ onBack }: PatientOTPFormProps) => {
  ...
  const [step, setStep] = useState<'email' | 'otp' | 'hospital'>('email');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Move to hospital selection step after OTP verification
      setStep('hospital');
  ...
  const handleHospitalSelection = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `You've selected ${selectedHospital.hospital_name}`,
      });
      navigate('/patient');
      ...
      {step === 'hospital' && (
        <>
          <div className="text-center space-y-2 pb-2">
            <h3 className="text-lg font-semibold">Select Your Hospital</h3>
            <p className="text-sm text-muted-foreground">
              Choose the hospital where you want to receive care
            </p>
          </div>
          ...
          {selectedHospital && (
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p className="text-sm text-blue-800">{selectedHospital.hospital_name}</p>
              {selectedHospital.district && (
                <p className="text-xs text-blue-600">{selectedHospital.district}, {selectedHospital.state}</p>
              )}
            </div>
          )}

          <div className="flex gap-3">
            <Button variant="outline" onClick={() => setStep('otp')} className="flex-1">Back</Button>
            <Button 
              onClick={handleHospitalSelection} 
              className="flex-1 gradient-primary text-primary-foreground" 
              disabled={loading || !selectedHospital}
            >
              {loading ? 'Processing...' : <><ArrowRight className="h-4 w-4 mr-2" /> Continue</>}
            </Button>
          </div>
        </>
      )}
